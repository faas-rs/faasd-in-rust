...
impl ContainerdProvider {
    pub async fn function_list(&self, namespace: String) -> Result<Vec<Status>, ListError> {
        let containers = self.docker.list_containers(Some(ListContainersOptions::<String> {
            all: true,
            ..Default::default()
        })).await.map_err(|e| ListError::Internal(e.to_string()))?;

        let mut out = Vec::new();
        for c in containers {
            let name = c.names.as_ref().and_then(|ns| ns.first()).cloned().unwrap_or_default();
            let name = name.strip_prefix('/').unwrap_or(&name);
            if !name.starts_with("faasdrs-") { continue; }
